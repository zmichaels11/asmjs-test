...
namespace graphics {
    ...
    vertex_array::vertex_array(const vertex_array_info& info) {                   
        ...
        if (info.pIndexBuffer) {
            glVertexArrayElementBuffer(_handle, info.pIndexBuffer->_handle);            
        }        

        for (int i = 0; i < info.bindingCount; i++) {
            auto pBinding = info.ppBindings[i];

            if (pBinding->divisor) {
                glVertexArrayBindingDivisor(_handle, pBinding->binding, pBinding->divisor);
            }

            if (pBinding->buffer) {
                glVertexArrayVertexBuffer(_handle, pBinding->binding, pBinding->buffer->_handle, pBinding->offset, pBinding->stride);
                std::printf("VertexArrayVertexBuffer(%d, %d, %d, %d, %d)\n", _handle, pBinding->binding, pBinding->buffer->_handle, pBinding->offset, pBinding->stride);
            }
        }        

        for (int i = 0; i < info.attributeCount; i++) {
            auto pAttrib = info.ppAttributes[i];
            
            glVertexArrayAttribFormat(_handle, pAttrib->location, _size(pAttrib->format), _type(pAttrib->format), _normalized(pAttrib->format), pAttrib->offset);            
            glVertexArrayAttribBinding(_handle, pAttrib->location, pAttrib->binding);

            std::printf("VertexArrayAttribFormat(%d, %d, %d, %x, %d, %d)\n", _handle, pAttrib->location, _size(pAttrib->format), _type(pAttrib->format), _normalized(pAttrib->format), pAttrib->offset);
            std::printf("VertexArrayAttribBinding(%d, %d, %d)\n", _handle, pAttrib->location, pAttrib->binding);
        }        
    }
